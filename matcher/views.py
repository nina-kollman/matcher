...
def search_job(request):
    ...
    print("candidates_list: ", candidates)
...
def sorted_candidates_for_all_jobs(job_obj, query):
    ...
    for job in job_obj:
        ...
        print("# all_ sorted", all_sorted)
        print("# job: ", job.job_skills.all())
...
def search_partial(request):
    ...
    query_dict = request.GET  # This is the input dict
    print(query_dict)
    query = query_dict.get("query")
    if query:
        candidates, job_found = find_partial_job_match(query)
        print("job FFF:", job_found)
